...
static void draw(Canvas* canvas, void* context) {
    ...
    canvas_draw_str(canvas, 2, 10, "Fire Phone Relay");
    canvas_set_font(canvas, FontSecondary);
    canvas_draw_str(canvas, 88, 10, app->connected ? "BT:OK" : "PAIR");
    canvas_draw_str(canvas, 2, 21, app->connected ? "Open Fire Remote app" : "Pair with iPhone BT");
    ...
    canvas_draw_str(canvas, 2, 53, "Arrows + OK = keys");
    canvas_draw_str(canvas, 2, 63, "Back=Esc Hold OK=Play");
}
...
static void bt_cb(BtStatus status, void* context) {
    FireRemoteApp* app = context;
    app->connected = (status == BtStatusConnected);
    if(app->view_port) view_port_update(app->view_port);
}

static void tap_kb(FireRemoteApp* app, uint16_t key) {
...
static void tap_consumer(FireRemoteApp* app, uint16_t key) {
    if(!app->hid_profile) return;
...
static void short_key(FireRemoteApp* app, InputKey key) {
    switch(key) {
    case InputKeyUp: tap_kb(app, FR_KEY_UP); break;
    case InputKeyDown: tap_kb(app, FR_KEY_DOWN); break;
    case InputKeyLeft: tap_kb(app, FR_KEY_LEFT); break;
    case InputKeyRight: tap_kb(app, FR_KEY_RIGHT); break;
    case InputKeyOk: tap_kb(app, FR_KEY_ENTER); break;
    case InputKeyBack: tap_kb(app, FR_KEY_ESCAPE); break;
    default: break;
    }
}

static void long_key(FireRemoteApp* app, InputKey key) {
    switch(key) {
    case InputKeyUp: tap_consumer(app, FR_CONSUMER_AC_HOME); break;
    case InputKeyDown: tap_consumer(app, FR_CONSUMER_MENU); break;
    case InputKeyLeft: tap_consumer(app, FR_CONSUMER_REWIND); break;
    case InputKeyRight: tap_consumer(app, FR_CONSUMER_FAST_FORWARD); break;
    case InputKeyOk: tap_consumer(app, FR_CONSUMER_PLAY_PAUSE); break;
...
int32_t fire_remote_app(void* p) {
    ...
    furi_delay_ms(200);
    bt_keys_storage_set_storage_path(app->bt, "/ext/apps_data/fire_remote/.bt_fire_relay.keys");

    static const BleProfileHidParams hid_params = {
        .device_name_prefix = "FireRelay",
        .mac_xor = 0xBEEF,
    };

    app->hid_profile = bt_profile_start(app->bt, ble_profile_hid, (void*)&hid_params);
    furi_check(app->hid_profile);
    bt_set_status_changed_callback(app->bt, bt_cb, app);
    ...
    while(app->running) {
        if(furi_message_queue_get(app->input_queue, &event, 100) == FuriStatusOk) {
            if(event.type == InputTypeShort) short_key(app, event.key);
            else if(event.type == InputTypeLong) long_key(app, event.key);
        }
    }
